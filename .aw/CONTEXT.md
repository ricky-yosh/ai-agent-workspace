...
- **Cargo workspace** with five crates: `core` (domain logic), `commands` (Command enum + executor), `cli` (CLI binary), `mcp` (MCP server library + optional Tauri plugin), `mcp-server` (standalone MCP binary). The Tauri app depends on `commands` and `mcp`. See ADR 0007 (workspace structure) and ADR 0009 (mcp/mcp-server crates).
