...
- **Terminal process exit** triggers an immediate auto-restart — treating `exit` as a terminal reset, not a termination. The PTY is bound to the panel node in the layout tree; it only truly stops when the panel node is removed.
