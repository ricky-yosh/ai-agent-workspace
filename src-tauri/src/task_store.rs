...
pub fn read_tasks(path: &Path) -> Result<Vec<Task>, String> {
    if !path.exists() {
        return Ok(Vec::new());
    }
...
pub fn add_task(path: &Path, input: TaskInput) -> Result<Task, String> {
...
pub fn update_task(path: &Path, task_id: u32, description: String, status: String) -> Result<Task, String> {
    let mut tasks = read_tasks(path)?;
    let task = tasks
        .iter_mut()
        .find(|t| t.id == task_id)
...
mod tests {
    ...
    fn test_add_task() {
        ...
        let task = add_task(&path, input).unwrap();
        assert_eq!(task.id, 1);
    ...
    fn test_update_task() {
        ...
        let added = add_task(&path, input).unwrap();
        let updated = update_task(&path, added.id, "Updated desc".into(), "completed".into()).unwrap();
    ...
    fn test_update_nonexistent() {
        let (_tmp, path) = setup();
        let result = update_task(&path, 999, "desc".into(), "completed".into());
        assert!(result.is_err());
    }
